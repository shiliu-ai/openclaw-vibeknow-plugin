...
/**
 * Single factory that returns ALL tools requiring PluginToolContext (im identity).
 * Matches the official memory-core pattern: one factory call, opts.names declared
 * at registration time so OpenClaw can discover them.
 */
export function createContextTools(opts: ContextToolsOptions) {
  const { client, callbackUrl, logger } = opts;
  ...
  return (ctx: PluginToolContext) => {
    ...
    logger.info(
      `[VibeKnow] factory called: im_handle=${im_handle ?? "<none>"}, im_channel=${im_channel ?? "<none>"}`,
    );
    ...
    return [
      // ── create_video_from_url ──
      {
        ...
        async execute(
          ...
          const uploadResult = await client.uploadUrl(params.url);
